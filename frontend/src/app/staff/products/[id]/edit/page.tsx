...
export default function StaffEditProductPage() {
  ...
  const { data: product, isLoading } = useQuery({
    queryKey: ['staff', 'products', productId],
  ...
  useEffect(() => {
    if (product) {
      setFormData({
        ...
        optionGroupIds: product.optionGroups?.map((g) => g.id) || [],
      });
      if (product.imageUrl) {
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: ProductFormData) => {
      const updated = await adminProductsApi.update(productId, data);
      if (imageFile) {
        await adminProductsApi.uploadImage(productId, imageFile);
      }
      return updated;
    },
    onSuccess: () => {
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">Chỉnh sửa sản phẩm</h1>
          <p className="text-sm text-gray-500">Cập nhật thông tin, giá bán và hình ảnh sản phẩm</p>
        </div>
      </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-6">
              ...
              <div>
                ...
                <p className="mt-1.5 text-xs text-gray-400">Định dạng JPG, PNG hoặc WEBP</p>
              </div>
            </div>
          </div>
